...
class KolmogorovSmirnovTest:
    def __init__(self, all_normal = True):
        self.all_normal = all_normal
        self.a_values = [0.2, 0.15, 0.1, 0.05, 0.01]
        num_samples = random.randrange(30, 300)  # Pois o teste Kolmogorov_Smirnov é recomendado para amostras > 30
        if all_normal == True:
            randomVar = np.random.normal(size=num_samples)
            self.bin_acerto = [1 for _ in range(len(self.a_values))]
        else:
            randomVar = np.random.poisson(size=num_samples)
            self.bin_acerto = [0 for _ in range(len(self.a_values))]
    ...
    def run_test(self):
        ...
        for alpha in self.a_values:  
            if self.N_samples <= 35:
                ...
                self.Dcrit = criticalV.loc[row_index][str(alpha)]
            else:
                match alpha:
                    case 0.2:
                        self.Dcrit = 1.07/math.sqrt(self.N_samples)
                    case 0.15:
                        self.Dcrit = 1.14/math.sqrt(self.N_samples)
                    case 0.1:
                        self.Dcrit = 1.22/math.sqrt(self.N_samples)
                    case 0.05:
                        self.Dcrit = 1.36/math.sqrt(self.N_samples)
                    case 0.01:
                        self.Dcrit = 1.63/math.sqrt(self.N_samples)
            self.Dcrit_values.append(round(self.Dcrit,3))  # Adicione o valor de Dcrit à lista
    ...
    def graficos(self):
        ...
        ax1.scatter(self.a_values,self.bin_acerto, color = 'blue' if self.all_normal == True else 'red')
        ax1.set_title('H0 e H1 esperado')
        ax1.set_ylabel("0 - Rejeita/ 1 - Aceita H0")
